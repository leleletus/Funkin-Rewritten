	...
	update = function(self, dt)
		...
		for i = 1, #events do
			if events[i].eventTime <= musicTime then
				...
				end  -- <-- Ensure this 'end' is present
		...
		if musicThres ~= oldMusicThres and math.fmod(absMusicTime, 240000 / bpm) < 100 then
			...
			camScaleTimer = Timer.tween(
				(60 / bpm) / 16, cam,
				{sizeX = camScale.x * 1.05, sizeY = camScale.y * 1.05},
				"out-quad",
				function()
					camScaleTimer = Timer.tween((60 / bpm), cam, {sizeX = camScale.x, sizeY = camScale.y}, "out-quad")
				end
			)
		end
		...
		-- La transición la gestiona weeks.handleSongEnd()
